...
### Визначення сайту провайдера

У результатах IP lookup поле **«Провайдер»** може містити назву організації,
клікабельне посилання на її сайт і favicon. Ця функція потрібна для того, щоб
показувати сайт власника ASN, а не випадковий домен конкретного сервера.

Сайт визначається у такому порядку:

1. ручна відповідність за ASN або назвою організації;
2. вбудований каталог для відомих провайдерів;
3. фоновий запит до BGP.Tools для ASN, якого ще немає в каталозі;
4. PeeringDB API, якщо BGP.Tools недоступний, вимагає авторизацію або не повернув сайт;
5. якщо сайт не вдалося підтвердити, показується лише назва провайдера без посилання.

Reverse DNS використовується тільки для відображення поля `rDNS`. Він навмисно
не використовується для визначення сайту провайдера, оскільки часто вказує на
домен клієнта, сервісу або окремого хоста.

Фонове визначення не затримує відповідь lookup: IP-дані показуються одразу,
а пошук сайту виконується окремим потоком. Результат зберігається у
`data/provider-overrides.json` і кешується в пам'яті. Успішні та невдалі перевірки
повторюються не частіше одного разу на 7 днів. Після знаходження сайту наступні
запити використовують кеш без звернення до зовнішніх сервісів.

Результати discovery записуються у `log/provider-discovery.log`. У кеші доступні
`status`, джерело (`discovery_source`), час перевірки (`checked_at`) і HTTP-код,
якщо зовнішній сервіс його повернув. Сайти зберігаються у JSON-кеші, а завантажені
favicon зберігаються локально у `data/provider-favicons/` і віддаються через
`/api/provider-favicon/{asn}`. Favicon оновлюється у фоні раз на 30 днів.

### Ручні відповідності

Ручні відповідності можна зберігати у `data/provider-overrides.json` або змінювати
через `PUT /api/provider-overrides` із заголовком
